...
def test_get_fill_price_branches_and_slippage(engine_base):
    ...
    # Market entry = OPEN
    assert e.get_fill_price({"symbol":"NQ","side":"buy"}, df, 0, 0.001) == 100.0

    # SL/TP use provided level then rounding
    p = e.get_fill_price(
        {"symbol":"NQ","side":"sell","stop_loss":100.12,"triggered_by_sl":True}, df, 0, 0.0
    )
    assert p == round_to_tick(100.12, 0.25)  # 100.0
    p = e.get_fill_price(
        {"symbol":"NQ","side":"sell","take_profit":100.13,"triggered_by_tp":True}, df, 0, 0.0
    )
    assert p == round_to_tick(100.13, 0.25)  # 100.25

    # Forced liquidation = CLOSE
    p = e.get_fill_price(
        {"symbol":"NQ","side":"sell","forced_liquidation":True,"closeout":True}, df, 0, 0.0
    )
    assert p == 101.0

    # Slippage precedence (ticks over pct), then rounding
    # Re-load with slippage enabled (mutate YAML safely)
    ...
    try:
        ...
        # Load existing config, mutate, and dump back
        with open(e.config_path, "r") as f:
            cfg = yaml.safe_load(f)
        cfg["include_slippage"] = True
        cfg["slippage_ticks"] = 2
        cfg["slippage_pct"] = 0.5
        # ensure the asset points to the new CSV we just wrote
        ...
        e.current_index = 0
        assert e.get_fill_price({"symbol":"NQ","side":"buy"}, e.get_asset_data("NQ"), 0, 0.5) == 100.5
        assert e.get_fill_price({"symbol":"NQ","side":"sell"}, e.get_asset_data("NQ"), 0, 0.5) == 99.5
    finally:
