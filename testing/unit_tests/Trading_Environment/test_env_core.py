...
def _mk_df(n=40, start=100.0):
    # Make sure we have enough history for BB(20), EMA(50), RSI(14), ATR(14)
...
def test_recompute_closed_bar_indicators_respects_cache(env_with_api, monkeypatch):
    ...
    # Spy on function to count calls
    import classes.Trading_Environment as mod
    calls = {"n": 0}
    orig = mod.update_indicators_at_index
    def spy(df_arg, idx_arg, indicator_config=None):
        calls["n"] += 1
        return orig(df_arg, idx_arg, indicator_config)
    monkeypatch.setattr(mod, "update_indicators_at_index", spy)
