...
MINUTE_AUGMENTED_INDICATOR_CONFIG = [
    ...
    #{'name': 'pullback', 'params': {'swing_k': 2, 'lookback_bars': 400, 'avg_n': 5}},
...
def _bars_since(series_bool: pd.Series) -> pd.Series:
    ...
    out = (np.arange(len(series_bool), dtype=float) - last)  # <- float, not int
    out[last < 0] = np.nan
...
def _compute_derived_policy_features(df: pd.DataFrame, tz="America/New_York") -> None:
    """
    Compute/ensure derived columns the PPO policy expects, using existing base indicators.
    Safe to call repeatedly; fills only missing columns.
    """
    if df is None or df.empty:
        return

    # --- Time / session: mins_since_open (if not already via indicator)
    if "mins_since_open" not in df.columns:
        ...
        rth_open = ts_local.dt.normalize() + pd.Timedelta(hours=9, minutes=30)
        mso = (ts_local - rth_open).dt.total_seconds() / 60.0
        df["mins_since_open"] = mso.clip(lower=0).astype(float)

    # --- ATR ratios
    if "atrR_90_390" not in df.columns and {"atr_90","atr_390"}.issubset(df.columns):
        df["atrR_90_390"] = _safe_div(df["atr_90"], df["atr_390"])
    if "atrRsm_14_60" not in df.columns and {"atr_14","atr_60"}.issubset(df.columns):
        df["atrRsm_14_60"] = _safe_div(df["atr_14"] - df["atr_60"], df["atr_60"])
    ...
    for w in (30, 60, 120):
        ...
        if pos not in df.columns and {low, wid}.issubset(df.columns):
            df[pos] = _safe_div(df["close"] - df[low], df[wid]).clip(0, 1)

    # --- ROC 60
    if "roc_60" not in df.columns and "close" in df.columns:
        df["roc_60"] = df["close"].pct_change(60)

    # --- Breakout age 60 (bars since a 60-bar breakout)
    if "breakout_age_60" not in df.columns and "close" in df.columns:
        ...
        df["breakout_age_60"] = _bars_since(trig)

    # --- Intraday ATR-normalized distances
    if {"high","low","close","atr_14"}.issubset(df.columns):
        ...
        if "iday_dist_to_high_atr" not in df.columns:
            df["iday_dist_to_high_atr"] = _safe_div(day_high - df["close"], df["atr_14"])
        if "iday_dist_to_low_atr" not in df.columns:
            df["iday_dist_to_low_atr"] = _safe_div(df["close"] - day_low, df["atr_14"])

    # --- Clean up non-feature columns if they sneak in
    for bad in ("sample_weight", "_et_min"):
...
class TradingEnvironment(QtCore.QObject):
    ...
    def set_api(self, api):
        ...
        # Pick up YAML flags from engine config (all optional)
        cfg = {}
        try:
            ...
            if self._precompute:
                ...
                for ind in cfg_list:
                    ...
                    if name == "ema":
                        ...
                        # fallback to prefix-based heuristic
                        if pref:
    ...
    def _recompute_closed_bar_indicators(self, _df=None):
        ...
        for symbol in self.get_asset_list():
            ...
            if self._indicator_cache.get(symbol) == idx:
                continue
            cfg = self._indicator_config_for_symbol(symbol)
            # PRE-DEFRAG before at-index updates
            self._defragment_asset_df(symbol)
            _router_update_at_index(df, idx, cfg, INDICATOR_REGISTRY)
            # Ensure derived features are updated for the new closed bar
            _compute_derived_policy_features(df)
            # POST-DEFRAG after updates
            self._defragment_asset_df(symbol)
    ...
    def get_latest_data(self, symbol, window_size=3): 
    ...
    def _compute_indicators(self, symbol, df, indicator_config=None):
        ...
        if not indicator_cols or (len(indicator_cols) > 0 and df[indicator_cols].isna().all().all()):
            ...
            # Ensure derived policy features after first full compute
            _compute_derived_policy_features(df)
            # CONSOLIDATE once after the first full compute
            self._defragment_asset_df(symbol)
            self._indicator_cache[symbol] = current_idx
        elif last_idx != current_idx:
            # PRE-DEFRAG before last-row updates
            self._defragment_asset_df(symbol)
            _router_update_last_row(df, indicator_config, INDICATOR_REGISTRY)
            # Ensure derived policy features for the last row
            _compute_derived_policy_features(df)
            # POST-DEFRAG after updates
            self._defragment_asset_df(symbol)
            self._indicator_cache[symbol] = current_idx
    ...
    def _defragment_asset_df(self, symbol: str) -> None:
        """
        Replace the backing DataFrame for `symbol` with a consolidated copy
        to avoid pandas block fragmentation warnings.
        """
        # If API doesn't use assets list, nothing to do
        ...
        for asset in self.api.assets:
            if asset.get("symbol") == symbol:
                ...
                if df is None:
                    break
                # Make a consolidated copy and store it back
                asset["data"] = df.copy()
                break
