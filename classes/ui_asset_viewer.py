...
class CandlestickChartWidget(QtWidgets.QWidget):
    ...
    def __init__(self, symbol, trading_env, parent=None):
        ...
        # --- Candlestick Items ---
        df_full = self.trading_env.get_latest_data(self.symbol)
    ...
    def auto_scroll_to_latest(self):
        df = self.trading_env.get_latest_data(self.symbol)
        if df.empty or 'timestamp' not in df.columns:
            return
...
class OrderEntryWidget(QtWidgets.QGroupBox):
    ...
    def __init__(self, symbol, trading_env, parent=None):
        ...
        self.sl_box = QtWidgets.QLineEdit()
        self.tp_box = QtWidgets.QLineEdit()
        ...
        layout.addRow("Stop Loss (ATR multiple):", self.sl_box)
        layout.addRow("Take Profit (optional):", self.tp_box)
    ...
    def _emit_order(self):
        ...
        atr_multiple_text = self.sl_box.text()
        take_profit_text = self.tp_box.text()
        order = {
            "symbol": self.symbol,
            "side": side,
            "qty": qty,
            "order_type": "market",
            "take_profit": float(take_profit_text) if take_profit_text else None,
        }

        # Only proceed if ATR multiple is given
        if atr_multiple_text:
            try:
                atr_multiple = float(atr_multiple_text)
            except Exception:
                QtWidgets.QMessageBox.warning(self, "Input Error", "ATR multiple must be a number.")
                return

            # Fetch the latest price and ATR
            df = self.trading_env.get_latest_data(self.symbol)
            if df is None or df.empty or "close" not in df.columns or "atr_14" not in df.columns:
                QtWidgets.QMessageBox.warning(self, "Data Error", "Cannot compute stop loss: missing price or ATR data.")
                return
            ...
            if side == "buy":
                stop_price = price - atr_multiple * atr
            else:
                stop_price = price + atr_multiple * atr

            order["stop_loss"] = stop_price

        # (Optionally: else warn if no stop loss at all is set)
