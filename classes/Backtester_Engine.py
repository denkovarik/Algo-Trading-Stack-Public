...
class BacktesterEngine(QtCore.QObject, APIInterface, metaclass=MetaQObjectABC):
    ...
    def _apply_global_defaults(self, config):
        ...
        # Skip synthetic bars during open phase
        self.skip_synthetic_open_bars = bool(config.get("skip_synthetic_open_bars", True))

        # Store data source (optional, TradeStation / Yahoo / etc.)
        self.data_source = config.get("data_source", None)

        # Store global per-asset contract cap
        self.max_contracts_per_asset = int(config.get("max_contracts_per_asset", 0))  
        # 0 = no cap, bots can check this later
        # Global timeframe default (e.g., "1m", "5m", "1h", "1d")
    ...
    def _build_symbol_params_and_load_assets(self, config):
        ...
        NEW:
          - Carry a per-asset `timeframe` (fallback to engine global self.timeframe).
          - Carry a per-asset `max_contracts` (fallback to engine global self.max_contracts_per_asset).
        """
        import os
        import pandas as pd
        ...
        # Pull global defaults (already set in _apply_global_defaults)
        ...
        for asset in assets_cfg:
            ...
            params = {
                ...
                # NEW: timeframe per asset (fallback to global)
                ...
                "maintenance_margin": float(asset.get(
                    "maintenance_margin",
                    asset.get("initial_margin", 0.0)
                )),
                "commission_per_contract": float(asset.get(
                    "commission_per_contract",
                ...
                # NEW: per-asset contract cap (0 or missing = no cap); bot may enforce
...
def load_tradestation_csv(filepath):
    ...
    and preserves any additional columns (e.g., precomputed indicators).
    """
    df = pd.read_csv(filepath)

    # Normalize core column names (keep any other columns untouched)
    df = df.rename(columns={
        "Open": "open",
        "High": "high",
        "Low": "low",
    ...
    # Ensure core time columns
    df["date"] = pd.to_datetime(df["date"], utc=True)
    df["timestamp"] = df["date"].astype("int64") // 10**9

    # Keep ALL columns: order core first, then any extras (indicators, features, etc.)
