...
class BacktesterEngine(QtCore.QObject, APIInterface, metaclass=MetaQObjectABC):
    ...
    # ---- Gap Detection (FIXED: Use asset['tick_size']) ----
    ...
    def _build_symbol_params_and_load_assets(self, config):
        ...
        for asset in assets_cfg:
            if asset.get("type") == "futures":
                ...
                if im is None or float(im) <= 0:
                    raise ValueError(f"[CONFIG ERROR] Asset {asset.get('symbol')} has invalid initial_margin: {im}")
        ...
        for asset in assets_cfg:
            ...
            params = {
                ...
                "initial_margin": float(asset.get("initial_margin")) if asset.get("initial_margin") is not None else None,
                "maintenance_margin": float(asset.get("maintenance_margin", asset.get("initial_margin", 0.0))),
                "commission_per_contract": float(asset.get("commission_per_contract", self.commission_per_contract)),
        ...
        self._detect_gaps()  # Run gap detection after loading
