...
from bots.exit_strategies import (
    ExitStrategy,
    TrailingATRExit,
    FixedRatioExit,          # imported for convenience (not required to use)
    StopLossToExitAdapter,   # legacy adapter
)

# NEW: central place to wrap on_bar with the exit’s online learner
from bots.online_learning import attach_online_learning
...
class BaseStrategyBot:
    ...
    def __init__(
        ...
        # NEW: opt-in online learning wrap (default on)
        ...
        if exit_strategy is not None:
            ...
            self.exit_strategy = StopLossToExitAdapter(stop_loss_strategy)
        else:
        ...
        # NEW: enable online learning after subclasses are bound
        # Idempotent; no effect if no on_bar or no ingest_trade_log.
        if enable_online_learning:
            attach_online_learning(self, verbose=True)
    ...
    def wants_entry_at(self, env, symbol, df, idx_last, idx_prev) -> bool:
        ...
        if idx_prev < 0 or idx_last >= len(df):
            return False
        if 'open' not in df.columns or pd.isna(df['open'].iloc[idx_last]):
            return False
        if 'close' not in df.columns or pd.isna(df['close'].iloc[idx_prev]):
            return False

        # Session/holiday rules (same as on_bar)
    ...
    def on_bar(self, env):
        ...
        for symbol in env.get_asset_list():
            ...
            if pos is None or pos.get('qty', 0) == 0:
                ...
                # If we already have a position, we still may want to manage exits below.
                pass
            ...
            entry_price = float(df['open'].iloc[idx_last])
            ...
            ctx = self._prepare_context(
                env=env, symbol=symbol, df=df, idx_last=idx_last, idx_prev=idx_prev,
                pos=pos, atr=atr, entry_price=entry_price, equity=equity,
                available_equity=available_equity, p=params
            )
            ...
            # --- Online learning hook (optional; no-op if not present) ---
            if hasattr(self.exit_strategy, "ingest_trade_log"):
                try:
                    self.exit_strategy.ingest_trade_log(env)
                except Exception as e:
                    # keep trading even if learner stumbles
                    if getattr(self.exit_strategy, "debug", False):
                        print(f"[SEQ-BANDIT] ingest_trade_log error: {e}")
    ...
    def _session_allows_entry(self, df, idx_last) -> bool:
        ts_et = self._to_eastern(df['date'].iloc[idx_last])
    ...
    def _handle_entry(self, ctx):
        ...
        # Allow exit strategy to lazy-load models once (e.g., RL-based)
    ...
    def _handle_exit(self, ctx):
        ...
        # Trail using the previous CLOSED bar
        current_price_for_trailing = float(df['close'].iloc[idx_prev])
    ...
    def _get_prev_atr(self, df: pd.DataFrame, idx_prev: int) -> float | None:
        ...
        if not atr_cols:
            return None
        val = df[atr_cols[0]].iloc[idx_prev]
