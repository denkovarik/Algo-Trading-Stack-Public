...
class BaseStrategyBot:
    ...
    def _get_prev_atr(self, df: pd.DataFrame, idx_prev: int) -> float | None:
        """
        Return the previous-bar ATR value.

        Priority:
          1) Any column explicitly listed in self.atr_preference (in order)
          2) Otherwise the first non-NaN column that starts with 'atr_'
             (column list cached per DataFrame columns signature)
        """
        if df is None or idx_prev < 0:
            return None

        # Try preferred names first (in order)
        if self.atr_preference:
            for name in self.atr_preference:
                if name in df.columns:
                    val = df[name].iloc[idx_prev]
                    if pd.notna(val):
                        return float(val)

        # Cache ATR candidates per columns signature (tuple of column names)
